...
local function addFavorite(animalName)
    ...
    if showNotification then
        showNotification("Ã¢Â­Â Added to favorites: " .. animalName, COLORS.Cyan)
    end
...
local function removeFavorite(animalName)
    for i, name in ipairs(FAVORITES) do
        if name:lower() == animalName:lower() then
            ...
            saveFavorites()
            if showNotification then
                showNotification("Ã¢ÂÅ’ Removed from favorites: " .. animalName, COLORS.Red)
            end
            return true
        end
    end
...
function ANTI_RAGDOLL.Disable()
    ...
    showNotification("Ã¢Å“â€” Anti-Ragdoll Disabled", COLORS.Red)
end
...
local function tpNearPlotIfFar(animalData)
    ...
    -- If close enough already Ã¢â€ â€™ skip
    ...
    -- Vector from plot center to player (playerÃ¢â‚¬â„¢s viewing side)
...
local function teleportToAnimal(animalData)
    ...
    if not character or not character:FindFirstChild("HumanoidRootPart") then
        showNotification("Ã¢Å¡Â Ã¯Â¸Â Character not found!", COLORS.Warning)
        return false
    end
    ...
    local plot = workspace.Plots:FindFirstChild(animalData.plot)
    if not plot then
        showNotification("Ã¢Å¡Â Ã¯Â¸Â Plot not found!", COLORS.Warning)
        return false
    end
    ...
    showNotification(string.format("Ã°Å¸Å¡â‚¬ Teleported to %s", animalData.name), COLORS.Cyan)
    return true
end
...
local function teleportToHighest()
    ...
    if CONFIG.FAVORITES_PRIORITY_ENABLED and #FAVORITES > 0 then
        ...
        if favAnimal then
            showNotification("Ã°Å¸â€â€“ Teleporting to priority: " .. favAnimal.name, COLORS.Yellow)
            teleportToAnimal(favAnimal)
            return
        else
            showNotification("Ã¢Å¡Â Ã¯Â¸Â No favorites found, using highest gen", COLORS.TextDim)
        end
    end
...
local function initializePlotScanner(statLabels)
    local plots = workspace:WaitForChild("Plots", 8)
    if not plots then
        warn("Ã¢Å¡Â Ã¯Â¸Â Plots folder not found!")
        return
    end
...
local function createAnimalEntry(animalData, parent)
    ...
    local name = createElement("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = (isFav and "Ã¢Â­Â " or "") .. animalData.name,
...
local function createFavoritesTab(parent)
    ...
    local apply = createElement("TextLabel", {
        ...
        Position = UDim2.new(0, 15, 0, 10),
        BackgroundTransparency = 1,
        Text = "Ã¢Â­Â Apply Favorites Priority",
        TextColor3 = COLORS.Yellow,
        TextSize = 15,
        Font = Enum.Font.GothamBold,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    toggleButton.MouseButton1Click:Connect(function()
        ...
        if showNotification then
            showNotification(
                CONFIG.FAVORITES_PRIORITY_ENABLED and "Ã¢Å“â€¦ Favorites Priority Enabled" or "Ã¢ÂÅ’ Favorites Priority Disabled",
